...
>
> Still open: Phase 5 (mobile), plus the Phase 1 leftovers (inline retry for swallowed fetch errors, bootstrap-error retry, pending-write indicators, in-modal team picker for the global create modal), and the ~2.3k pre-existing raw-color occurrences the Phase 3 codemod didn't touch (guarded against regression, not yet eliminated).
