...
**Request correlation (AsyncLocalStorage).** `/api/graphql` wraps each request in `runWithRequestContext({ requestId, orgId, userId }, …)`. A pino `mixin` merges that store onto **every** log line emitted during the request — including deep in services — so no logger needs to be threaded through call sites. To add request-scoped fields elsewhere, call `runWithRequestContext` (it shallow-merges with the parent scope).
