...
/**
 * Completes the Google OAuth login. Google redirects here with `code` and
...
export function GoogleCallbackHandler() {
  ...
  useEffect(() => {
    ...
    async function exchange() {
      ...
      try {
        const result = await gql(GOOGLE_AUTH_EXCHANGE_MUTATION, { code, state });

        const payload = (
          result.data as {
            googleAuthExchange?: { accessToken: string; refreshToken: string };
          } | null
        )?.googleAuthExchange;
        if (!payload) {
          setError(gqlError(result, 'Google sign-in failed. Please try again.'));
          return;
        }
  ...
  return (
    <div className="flex flex-col items-center gap-4 text-center">
      {error ? (
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400">Signing you in with Google…</p>
      )}
    </div>
  );
}
