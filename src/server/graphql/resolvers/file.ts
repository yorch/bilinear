...
import { FileForbiddenError, FileNotFoundError } from '../../services/file.service';
...
export const fileResolvers = {
  ...
  Mutation: {
    fileDelete: async (_parent: unknown, args: { id: string }, ctx: GraphQLContext) => {
      requireAuth(ctx);
  ...
  Query: {
    issueFiles: (_parent: unknown, args: { issueId: string }, ctx: GraphQLContext) => {
      requireAuth(ctx);
